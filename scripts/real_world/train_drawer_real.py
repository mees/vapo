...
def main(cfg):
    ...
    model = VAPOAgent(
        cfg, sac_cfg=sac_cfg, target_search_mode=cfg.target_search, wandb_login=cfg.wandb_login, rand_target=True
    )
    if cfg.resume_training:
        original_dir = hydra.utils.get_original_cwd()
        model_path = os.path.join(original_dir, cfg.resume_model_path)
        path = "%s/trained_models/%s.pth" % (model_path, cfg.model_name + "_last")
