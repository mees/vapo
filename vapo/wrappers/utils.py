...
def find_cam_ids(cameras):
    ...
    for i, cam in enumerate(cameras):
        if "gripper" in cam.name:
            ...
            render_cam_id = i
    cameras = {"static": static_id, "gripper": gripper_id}
    if render_cam_id:
    ...
    for cam_type, config in cfg_dict.items():
        ...
        if env_obs_cfg.use_img:
            obs_space_dict["%s_img_obs" % cam_type] = gym.spaces.Box(
                low=0, high=255, shape=(channels, img_size, img_size)
            )
        if env_obs_cfg.use_depth:
            obs_space_dict["%s_depth_obs" % cam_type] = gym.spaces.Box(low=0, high=255, shape=(1, img_size, img_size))
        if aff_cfg.use:
            obs_space_dict["%s_aff" % cam_type] = gym.spaces.Box(low=0, high=1, shape=(1, img_size, img_size))
    if use_robot_obs:
        # *tcp_pos(3), *tcp_euler(1), gripper_width, gripper_action(1),
        if task == "pickup":
            obs_space_dict["robot_obs"] = gym.spaces.Box(low=-0.5, high=0.5, shape=(6,))
        else:
            if real_world:
                # pos +  width
                obs_shape = 4
            ...
            obs_space_dict["robot_obs"] = gym.spaces.Box(low=-0.5, high=0.5, shape=(obs_shape,))
    if affordance_cfg.gripper_cam.target_in_obs:
        obs_space_dict["detected_target_pos"] = gym.spaces.Box(low=-1, high=1, shape=(3,))
    if affordance_cfg.gripper_cam.use_distance:
        obs_space_dict["target_distance"] = gym.spaces.Box(low=-1, high=1, shape=(1,))
    return gym.spaces.Dict(obs_space_dict)


def depth_preprocessing(frame, img_size):
    # obs is from 0-255, (img_size, img_size, 1)
    new_frame = cv2.resize(frame, (img_size, img_size), interpolation=cv2.INTER_AREA)
    # (1, img_size, img_size)
