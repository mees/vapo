...
class Robot:
    ...
    def relative_to_absolute(self, action):
        ...
        if self.use_target_pose:
            ...
            tcp_pos, tcp_orn = p.getLinkState(self.robot_uid, self.tcp_link_id, physicsClientId=self.cid)[:2]
            t_matrix = orn_to_matrix(tcp_orn) @ orn_to_matrix([np.pi, 0, np.pi / 2])
            # Transform displacement to global frame
            rel_disp_g = t_matrix @ rel_pos
            # Target position is in global frame
            _target_pos = self.target_pos + rel_disp_g[:3]
            _max_pos = tcp_pos + self.max_drift
            _min_pos = tcp_pos - self.max_drift
            _new_pos = np.clip(_target_pos, _min_pos, _max_pos)

            self.target_pos = _new_pos
            self.target_orn += rel_orn[:3]
            # if((diff < self.max_drift).all()):
            #     self.target_pos = _target_pos
            #     self.target_orn += rel_orn[:3]

            # p.removeAllUserDebugItems()
            # p.addUserDebugText("t", textPosition=self.target_pos, textColorRGB=[1, 0, 0])
            # p.addUserDebugText("p", textPosition=tcp_pos, textColorRGB=[0, 0, 1])
            return self.target_pos, self.target_orn, gripper
        else:
            tcp_pos, tcp_orn = p.getLinkState(self.robot_uid, self.tcp_link_id, physicsClientId=self.cid)[:2]

            rel_pos = orn_to_matrix(tcp_orn) @ orn_to_matrix([np.pi, 0, 0]) @ rel_pos
